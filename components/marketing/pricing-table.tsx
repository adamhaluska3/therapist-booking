...
const columns = [
  columnHelper.accessor("name", {
    header: "Služba",
    cell: (info) => (
      <p className="font-serif text-lg font-medium text-brand-900">{info.getValue()}</p>
    ),
  }),
    ...
    <>
      <div className="sm:hidden divide-y divide-surface-100">
        {table.getRowModel().rows.map((row) => {
          ...
          const price = service.price != null ? `${Math.round(service.price / 100)}€` : "—"
          const isBookable = service.name.toLowerCase() === "psychoterapia"
          return (
            <div key={row.id} className={isBookable ? "bg-brand-50/40" : ""}>
              <div className="px-6 pt-6 pb-4">
                <p className="font-serif text-2xl font-medium text-brand-900">{service.name}</p>
              </div>
              <div className="flex gap-3 px-6 pb-5">
                <div className="flex-1 rounded-xl border border-surface-200 bg-white px-4 py-3 text-center">
                  ...
                  <p className="text-sm font-semibold text-neutral-700">{DEFAULT_DURATION_MINUTES} min</p>
                </div>
                <div className="flex-1 rounded-xl border border-surface-200 bg-white px-4 py-3 text-center">
                  <p className="text-xs text-neutral-400 mb-1">Cena</p>
                  <p className="text-sm font-semibold text-neutral-700">{price}</p>
                </div>
              </div>
