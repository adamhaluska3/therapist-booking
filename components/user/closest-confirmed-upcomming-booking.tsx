...
export const ClosestConfirmedUpcommingBooking = ({
  item,
}: ClosestConfirmedUpcommingBookingProps) => (
  <article className="flex bg-white rounded-2xl p-10 overflow-hidden gap-10 h-full">
    <section className="flex flex-col gap-5 w-full">
      <div className="flex flex-1 flex-col gap-5">
        <div className="flex justify-start gap-2 items-center">
          <span className="p-2 bg-brand-300 text-brand-800 rounded-2xl font-bold uppercase text-xs">
            Potvrdené
          </span>
        ...
        <div className="flex justify-center">
          <Image
            ...
            src={
              bookingTypeImageSrc[
                "bt-psychoterapia" as keyof typeof bookingTypeImageSrc
              ]
            }
            alt=""
            width={280}
        ...
        <div className="flex gap-2 text-sm text-brand-700">
          <Clock5 />
          <span>
            {item.start.toLocaleTimeString("sk-SK", {
              hour: "2-digit",
              minute: "2-digit",
            })}{" "}
            -{" "}
            {item.end.toLocaleTimeString("sk-SK", {
              hour: "2-digit",
              minute: "2-digit",
            })}
          </span>
      ...
      <div className="flex flex-col sm:flex-row gap-2 text-sm text-brand-700 w-full">
        ...
        <ClientPaymentInfo
          ...
          centPrice={item.price || 0}
          note={item.bookingType?.name || ""}
        >
