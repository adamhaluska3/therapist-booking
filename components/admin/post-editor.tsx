...
import { createPostAction, updatePostAction } from "@/server/blog/actions";
...
const TitleImageUpload = dynamic(() => import("./title-image-upload"), {
  ssr: false,
});
...
export const PostEditor = ({ post, categories }: PostEditorProps) => {
  ...
  const { mutate, isPending } = useMutation({
    ...
    onError: (e) => {
      toast.error(
        e instanceof Error ? e.message : "Nepodarilo sa uložiť príspevok.",
      );
    },
    onSuccess: () => {
      ...
      <form>
        <div className="flex flex-col lg:flex-row gap-5">
          <div className="flex-1">
            <div className="flex flex-col">
              <label
                htmlFor="title"
                className="mb-4 text-xs font-semibold uppercase tracking-widest text-brand-600"
              >
                Názov
              </label>
              <input
        ...
        <div className="mt-5 flex flex-col sm:flex-row justify-end gap-3">
          ...
          <Button
            ...
            className={cn(
              "mt-5 py-2 px-5 bg-brand-500 rounded-2xl text-white text-xs font-semibold uppercase tracking-widest",
