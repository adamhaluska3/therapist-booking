...
const TitleImageUpload = ({ onChange, existingUrl, className }: { 
  ...
  className?: string
    ...
    const currentPreview = files.length > 0 ? URL.createObjectURL(files[0]) : existingUrl
    ...
    return (
        <div className={className ?? ''}>
            {hasImage ? (
                ...
                <FileUpload value={files} onValueChange={handleValueChange} accept="image/*" maxFiles={1}>
                    <FileUploadDropzone className="border-dashed border-2 rounded-xl p-6 text-center">
                        ...
                        <p className="text-sm text-muted-foreground mt-2">alebo pretiahnite sem</p>
                    </FileUploadDropzone>
                    <FileUploadList />
                </FileUpload>
            )}
