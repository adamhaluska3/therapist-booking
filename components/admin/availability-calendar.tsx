...
const DnDCalendar = withDragAndDrop<TherapistEvent>(
  Calendar as ComponentType<CalendarProps<TherapistEvent>>,
);
...
function DayColumnHeader({ date }: { date: Date; label: string }) {
  ...
  return (
    <div className="py-2 text-center">
      <div
        className={`text-[10px] uppercase tracking-widest font-medium ${today ? "text-brand-500" : "text-gray-400"}`}
      >
        {abbr}
      </div>
      <div
        className={`text-base leading-tight ${today ? "text-brand-600 font-bold" : "text-gray-700 font-light"}`}
      >
        {num}
      </div>
    </div>
...
export function AvailabilityCalendar({
  ...
  const openSlot = slotDialogId
    ? slots.find((s) => s.id === slotDialogId)
    : undefined;
  ...
  function applyAndPersistSlotChange(
  ...
  const handleSlotCreateBooking = useCallback(
  ...
  const handleNewEventSlot = useCallback(
    (start: Date, end: Date) => {
      const id = crypto.randomUUID();
      applyAndPersistSlotChange(
        [...slots, { id, start, end, label: null }],
  ...
  const handleNavigate = useCallback(
    async (action: "PREV" | "NEXT" | "TODAY") => {
      ...
      const anchor =
        view === "week" ? startOfWeek(base, { weekStartsOn: 1 }) : base;
      ...
      try {
        ...
        persistedSlotIds.current = new Set(
          newSlots.map((s: AvailabilitySlot) => s.id),
        );
        persistedBookingIds.current = new Set(
          newBookings.map((b: BookingWithUser) => b.id),
        );
      } catch {
  ...
  const interactiveAccessor = useCallback(
    (event: TherapistEvent) =>
      !isMobile && (event.source === "booking" || event.isDraggable === true),
    [isMobile],
  );
    ...
    return (
      <div className="max-w-5xl mx-auto">
        ...
        <div
          ...
          className="rounded-xl bg-surface-50 border border-surface-200"
        />
      </div>
    );
  }
